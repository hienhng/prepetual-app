...
- **Progress Page**: Replaced the AccuracyDialog with a dedicated full page at `/progress`. Features: stats summary banner (avg accuracy, total attempts, trend), tabbed interface with Overview (line chart with per-subject colored lines, score distribution), Subjects (per-category stats cards with average and best scores), and All Results (scrollable list of all past quiz results with category badges). Line chart groups data by date and shows different colored lines per quiz category (Math, English, Science, etc.). Backend result-history API now includes quiz category. Sidebar has "Progress" link with ChartLine icon. Dashboard accuracy stat card navigates to /progress.
