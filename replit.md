...
- **Folder Organization**: Users can now organize quizzes into folders on the Archive page. Features include: creating/renaming/deleting folders, moving quizzes into folders via a dropdown menu, filtering quizzes by folder (including "Unfiled"), and folder count badges. Folders are stored in a `folders` table with a `folderId` foreign key on quizzes (set null on folder delete). Backend has full CRUD routes for folders and updated quiz update to support folderId assignment.
